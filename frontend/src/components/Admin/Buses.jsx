...
const Buses = ({ username }) => {
  ...
  const fetchBuses = async () => {
    try {
      const response = await axios.get('http://localhost:5000/api/buses/fetchbuses',{withCredentials:true});
  ...
  const handleAddBus = async () => {
    try {
      const response = await axios.post('http://localhost:5000/api/buses/addbus', newBus,{withCredentials:true});
      if (response.status === 201) {
        alert('Bus added successfully!');
        fetchBuses(); // Refresh the buses list after adding
      } else {
  ...
  // Handle input change for adding bus
    ...
    <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
      ...
      <div className="flex-1 p-6 md:ml-64">
        ...
        <div className="p-6 mb-6 bg-white rounded-lg shadow-lg">
          <h2 className="mb-4 text-xl font-bold text-indigo-600">Add Bus</h2>
        ...
        <div className="p-6 bg-white rounded-lg shadow-lg">
          <h2 className="mb-4 text-xl font-bold text-indigo-600">Available Buses</h2>
          {buses.length > 0 ? (
            <ul className="space-y-4">
              {buses.map((bus) => (
                <li key={bus._id} className="p-4 bg-gray-100 rounded-lg">
                  <p><strong>Operator:</strong> {bus.operator}</p>
                  <p><strong>Bus Number:</strong> {bus.busNumber}</p>
                  <p><strong>Type:</strong> {bus.type}</p>
                  <p><strong>Seats:</strong> {bus.seats}</p>
                  <p><strong>Amenities:</strong> {bus.amenities}</p>
                </li>
              ))}
            </ul>
          ) : (
